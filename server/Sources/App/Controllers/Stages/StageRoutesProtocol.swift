...
protocol StageRoutesProtocol {
    ...
    var stageRoute: StageRoutes {get}
...
extension StageRoutesProtocol {
    ...
    func setupRoutesBuilder(routes: RoutesBuilder){
        ...
        let stage = routes.grouped(PathComponent(stringLiteral: stageRoute.route.main))
        
        stage.post(use: insertStage)
        stage.get(use: fetchAllStages)
        
        //With stageId
        stage.group(PathComponent(stringLiteral: stageRoute.route.id)) { (stage) in
