...
class DataManager: DataManagerLogic{
    ...
    internal func createContainer(containerInput: StagesContainer.Inoutput,req: Request) throws -> EventLoopFuture<HTTPStatus>{
        ...
        let newContainer = StagesContainer(id: containerInput.id, type: containerInput.type, stages: containerInput.stages.map({$0.rawValue}), farmId: containerInput.farmId, name: containerInput.name, desc: containerInput.desc, image: containerInput.image)
